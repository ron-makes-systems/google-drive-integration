Hey, I want you to develop integration for my app based on the guide below + a real-world example in @examples/chargebee-connector. Here's Fibery Integration Guide to help you.
...
- The @src folder already contains a lot of boilerplate code to get you started
- @tests also contains the first /status test as well as some test utils
- Start with minimal USEFUL set of objects required to put the first data in Fibery, and then develop the solution from here
- Tip: search for "your-connector" or "Your Connector" to find places where template should be extended with integration-specific code
- Include tests for new functionality/objects
- If integration uses SDK, look in the code of the SDK what fields are accessible for object you are currently working on and what fields would be useful to include

# Research Results
Here's research from Claude on API of my external app and quirks you might face during development. Rely on that during your decision making:
